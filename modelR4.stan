...
parameters {
  ...
  vector[S] beta1;       // 周期成分の推定値
  vector[S] beta2;       // 周期成分の推定値
  vector[S] beta3;       // 周期成分の推定値
...
model {
  ...
  beta1[1] ~ normal(beta1[S], s_b1);  // 現存量の過程誤差の遷移
  beta2[1] ~ normal(beta2[S], s_b2);  // 現存量の過程誤差の遷移
  beta3[1] ~ normal(beta3[S], s_b3);  // 現存量の過程誤差の遷移
  ...
  s_b1 ~ normal(0,1); //0.0005not good
  s_b2 ~ normal(0,1); //0.0005not good
  // s_b3 ~ normal(0,0.1); //0.0005not good
}
